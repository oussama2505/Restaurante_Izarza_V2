...
/**
 * Invoice template for take-away orders.
 *
 * @since 1.9
 */
class TakeawayInvoiceTemplate extends OrderInvoiceTemplate
{
	/**
	 * @inheritDoc
	 */
	protected function getPageTemplate()
	{
		...
		$layout = new \JLayoutFile('templates.invoice.takeaway', null, [
	...
	protected function parseTemplate($tmpl, &$data)
	{
		...
		$tmpl = str_replace(    '{invoice_grandtotal}', $currency->format($this->order->total_to_pay)                          , $tmpl);
		$tmpl = str_replace(     '{invoice_paycharge}', $currency->format($this->order->payment_charge)                        , $tmpl);
		$tmpl = str_replace('{invoice_deliverycharge}', $currency->format($this->order->delivery_charge)                       , $tmpl);
