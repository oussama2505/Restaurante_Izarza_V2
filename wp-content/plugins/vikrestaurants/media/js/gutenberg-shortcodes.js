(function(wp) {
	 ...
	 */
	registerBlockType('vikrestaurants/gutenberg-shortcodes', {
		/**
		 * This is the block display title, which can be translated with `i18n` functions.
		 * The block inserter will show this name.
		 */
		title: __('VikRestaurants Shortcode', 'vikrestaurants'),

		/**
		 * This is the block description, which is displayed within the right sidebar.
		 */
		description: __('Add a shortcode configured through VikRestaurants.', 'vikrestaurants'),
		...
		edit: (props) => {
			...
			// iterate vikrestaurants shortcodes to build select options
			let options = [];
			...
			// insert empty option
			options.push({
				label: __('- pick a shortcode -', 'vikrestaurants'),
				value: '',
			});

			// insert an option to support the creation of new shortcodes
			options.push({
				label: __('- create a new shortcode -', 'vikrestaurants'),
				value: 'new',
			});
			...
			for (let group in VIKRESTAURANTS_SHORTCODES_BLOCK.shortcodes) {
				let groups = VIKRESTAURANTS_SHORTCODES_BLOCK.shortcodes[group];
				...
				for (let i = 0; i < groups.length; i++) {
					...
					if (togglerChecked || !post_id || post_id === wp.data.select('core/editor').getCurrentPostId() || data.shortcode == currentShortcode) {
						...
						if (post_id && data.shortcode != currentShortcode) {
							assigneeField = el(
								'a',
								{
									...
									onClick: () => {
										alert(__('This shortcode is already used by a different post. If you select this shortcode, it will be detached from the existing post and assigned to this new post.', 'vikrestaurants'));
									},
								},
								el(
									'span',
									...
									__('Post #', 'vikrestaurants') + post_id
								)
							);
						} else {
							// safe shortcode
							assigneeField = el('span', {}, (post_id ? __('Post #', 'vikrestaurants') + post_id : '--'));
						}
						...
						shortcodes_boxes.push(el(
							'div',
							{
								className: 'vikrestaurants-shortcode-info-box' + (toggled ? ' toggled' : ''),
							},
							// create child elements
							el(
								'div',
								{
									className: 'vre-sh-info-control'
								},
								[
									el('label', {}, __('Type:', 'vikrestaurants')),
									el('span', {}, group),
								]
							),
							el(
								...
								[
									el('label', {}, __('Name:', 'vikrestaurants')),
									el('span', {}, data.name),
								]
							),
							el(
								...
								[
									el('label', {}, __('Created on:', 'vikrestaurants')),
							...
							el(
								...
								[
									el('label', {}, __('Assignee:', 'vikrestaurants')),
									assigneeField
								]
							),
							el(
								...
								el(
									...
									{
										label: __('Shortcode:', 'vikrestaurants'),
										value: data.shortcode,
										readonly: true,
									}
								)
			...
			navBar.push(
				el(
					...
					el(
						...
						{
							label: __('See all', 'vikrestaurants'),
							checked: togglerChecked,
							help: togglerChecked ? __('Display all the existing shortcodes.', 'vikrestaurants') : __('Toggle to display also the shortcodes that are already assigned to a post.', 'vikrestaurants'),
							onChange: (toggler) => {
			...
			if (props.attributes.shortcode === 'new') {
				...
				if (!newShortcodeRegistry) {
					...
					newShortcodeRegistry = new VREBlockPropertiesDecorator({lang: '*'}, props);
				} else {
				...
				newItemPanelFields.push(
					el(
						...
						{
							label: __('Page Type', 'vikrestaurants'),
							help: __('Choose the type of the page that should be displayed in the front-end.', 'vikrestaurants'),
							value: newShortcodeRegistry.attributes.type,
							options: [
								{
									label: __('Select an option', 'vikrestaurants'),
									value: '',
								}
							].concat(VIKRESTAURANTS_SHORTCODES_BLOCK.views.map((page) => {
								return {
									label: page.name,
									value: page.type,
								};
				...
				newItemPanelFields.push(
					el(
						wp.components.SelectControl,
						{
							label: __('Language', 'vikrestaurants'),
							help: __('Choose whether the shortcode should be available for a specific language only.', 'vikrestaurants'),
							value: newShortcodeRegistry.attributes.lang,
							options: [
								{
									label: __('All', 'vikrestaurants'),
									value: '*',
								}
							].concat(VIKRESTAURANTS_SHORTCODES_BLOCK.languages.map((lang) => {
								return {
									label: lang.name,
									value: lang.tag,
								};
				...
				let selectedView = VIKRESTAURANTS_SHORTCODES_BLOCK.views.filter(view => view.type === newShortcodeRegistry.attributes.type).shift();
				...
				if (selectedView) {
					selectedView.fields.forEach((field) => {
						...
						let fieldControl = window.vreCreateControlElement(field, newShortcodeRegistry);
				...
				navBar.push(
					el(
						wp.components.PanelBody,
						{
							title: __('New Item', 'vikrestaurants'),
							initialOpen: true,
						},
						newItemPanelFields
					)
				...
				newItemPanelFields.push(
					el(
						...
						{
							text: __('Create Shortcode', 'vikrestaurants'),
							isPrimary: true,
							onClick: (event) => {
								...
								if (parentId) {
									request.parent_id = (
										...
										Object.values(VIKRESTAURANTS_SHORTCODES_BLOCK.shortcodes).flat().filter(shortcode => shortcode.post_id == parentId).shift()
								...
								new Promise((resolve, reject) => {
									...
									if (!request.name) {
										reject(__('You need to specify a title for this post first.', 'vikrestaurants'));
										return;
									}

									// make request to save the shortcode
									jQuery.ajax({
										url: VIKRESTAURANTS_SHORTCODES_BLOCK.ajaxurl,
										type: 'post',
										data: request,
									}).done((shortcode) => {
										resolve(shortcode);
									...
									if (VIKRESTAURANTS_SHORTCODES_BLOCK.shortcodes.custom === undefined) {
										VIKRESTAURANTS_SHORTCODES_BLOCK.shortcodes.custom = [];
									}

									// append the newly created shortcode to the custom slot
									VIKRESTAURANTS_SHORTCODES_BLOCK.shortcodes.custom.push(shortcode);

									// update the selected shortcode
									props.setAttributes({shortcode: shortcode.shortcode});
								}).catch((error) => {
									// something went wrong, alert the user
									alert(error || __('An error has occurred', 'vikrestaurants'));
			...
			if (props.attributes.shortcode === 'new') {
				renderer = el(
					...
					__('You can create a new shortcode from the block settings under the right sidebar.', 'vikrestaurants')
				);
			} else {
				renderer = el(
					wp.serverSideRender,
					{
						block: 'vikrestaurants/gutenberg-shortcodes',
			...
			return [
				...
				el(
					...
					{
						className: 'vre-shortcode-admin-wrapper',
					},
					// <div> contains select
					[
						el(
	 ...
	 */
	const VREBlockPropertiesDecorator = class VREBlockPropertiesDecorator {
