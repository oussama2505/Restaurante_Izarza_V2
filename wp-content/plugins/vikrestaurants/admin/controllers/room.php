...
class VikRestaurantsControllerRoom extends VREControllerAdmin
{
	...
	public function add()
	{
		...
		$app->setUserState('vre.room.data', []);
	...
	public function save($copy = false)
	{
		...
		$this->setRedirect('index.php?option=com_vikrestaurants&task=room.edit&cid[]=' . $id);
	...
	public function publish()
	{
		...
		// check user permissions
		if (!$user->authorise('core.edit.state', 'com_vikrestaurants') || !$user->authorise('core.access.rooms', 'com_vikrestaurants'))
		{
			// back to main list, not authorised to edit records
			$app->enqueueMessage(JText::translate('JERROR_ALERTNOAUTHOR'), 'error');
			$this->cancel();
