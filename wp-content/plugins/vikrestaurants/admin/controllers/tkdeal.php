...
class VikRestaurantsControllerTkdeal extends VREControllerAdmin
{
	...
	public function add()
	{
		...
		$app->setUserState('vre.tkdeal.data', []);
	...
	public function save()
	{
		...
		$args['shifts']       = $input->get('shifts', [], 'uint');
		$args['type']         = $input->get('type', '', 'string');
		$args['days']         = $input->get('days', [], 'uint');
		$args['id']           = $input->get('id', 0, 'uint');

		// obtain all the parameters, products and gifts set in request
		$params   = $input->get('params', [], 'array');
		$products = $input->get('products', [], 'array');
		$gifts    = $input->get('gifts', [], 'array');

		// take only the ones that match the selected deal type
		$args['params']   = $params[$args['type']] ?? [];
		$args['products'] = $products[$args['type']] ?? [];
		$args['gifts']    = $gifts[$args['type']] ?? [];

		// fetch service
		$delivery = $input->getBool('delivery', false);
		$takeaway = $input->getBool('takeaway', false);
		...
		$args['start_ts'] = DateHelper::getTimestamp($args['start_ts']);
		$args['end_ts']   = DateHelper::getTimestamp($args['end_ts']);

		// also register the deleted products to properly recover them in case of failure
		$args['deleted_products'] = $input->get('products_deleted', [], 'uint');
		$args['deleted_gifts']    = $input->get('gifts_deleted', [], 'uint');
		...
		$this->getModel('tkdealproduct')->delete($args['deleted_products']);
		$this->getModel('tkdealfree')->delete($args['deleted_gifts']);

		// display generic successful message
		$app->enqueueMessage(JText::translate('JLIB_APPLICATION_SAVE_SUCCESS'));

		// redirect to edit page
		$this->setRedirect('index.php?option=com_vikrestaurants&task=tkdeal.edit&cid[]=' . $id);
	...
	public function publish()
	{
		...
		// check user permissions
		if (!$user->authorise('core.edit.state', 'com_vikrestaurants') || !$user->authorise('core.access.tkdeals', 'com_vikrestaurants'))
		{
			// back to main list, not authorised to edit records
			$app->enqueueMessage(JText::translate('JERROR_ALERTNOAUTHOR'), 'error');
			$this->cancel();
