...
class VikRestaurantsViewtkareas extends JViewVRE
{
	...
	function display($tpl = null)
	{
		...
		$q = $dbo->getQuery(true)
			...
			->from($dbo->qn('#__vikrestaurants_takeaway_delivery_area', 'a'))
		...
		if ($filters['type'])
		{
			$q->where($dbo->qn('a.type') . ' = ' . $dbo->q($filters['type']));
		}

		/**
		 * It is possible to lean on the "onBeforeListQueryTkareas" plugin event
		...
		$this->supportedTypes = E4J\VikRestaurants\DeliveryArea\Factory::getSupportedTypes();
	...
	private function addToolBar()
	{
		...
		JToolbarHelper::title(JText::translate('VRMAINTITLEVIEWTKAREAS'), 'vikrestaurants');

		$user = JFactory::getUser();
		
		if ($user->authorise('core.create', 'com_vikrestaurants'))
		{
			JToolbarHelper::addNew('tkarea.add');
		}

		if ($user->authorise('core.edit', 'com_vikrestaurants'))
		{
			JToolbarHelper::editList('tkarea.edit');
			JToolbarHelper::publishList('tkarea.publish');
			JToolbarHelper::unpublishList('tkarea.unpublish');
		}

		if ($user->authorise('core.delete', 'com_vikrestaurants'))
		{
			JToolbarHelper::deleteList(VikRestaurants::getConfirmSystemMessage(), 'tkarea.delete');
		}
	}
