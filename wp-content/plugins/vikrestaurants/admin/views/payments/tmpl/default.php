...
if ($canOrder && $canEditState)
{
	...
	JHtml::fetch('vrehtml.scripts.sortablelist', 'paymentsList', 'adminForm', $this->orderDir, $saveOrderingUrl);
}
		...
		<?php echo $vik->openTableHead(); ?>
			<tr>
				...
				<th class="<?php echo $vik->getAdminThClass('left'); ?>" width="15%" style="text-align: left;">
					<?php echo JHtml::fetch('vrehtml.admin.sort', 'VRMANAGEPAYMENT1', 'p.name', $this->orderDir, $this->ordering); ?>
				</th>

				<!-- FILE -->
				
				<th class="<?php echo $vik->getAdminThClass('left hidden-phone'); ?>" width="15%" style="text-align: left;">
					<?php echo JText::translate( 'VRMANAGEPAYMENT2' ); ?>
				</th>

				<!-- COST -->
				
				<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="5%" style="text-align: center;">
					<?php echo JText::translate( 'VRMANAGEPAYMENT4' ); ?>
				</th>
			...
			<tr class="row<?php echo ($i % 2); ?>">
				...
				<td>
					<div class="td-primary">
						...
						{
							?>
							<a href="index.php?option=com_vikrestaurants&amp;task=payment.edit&amp;cid[]=<?php echo (int) $row['id']; ?>">
								<?php echo $row['name']; ?>
							</a>
							<?php
						}
				...
				<td style="text-align: center;">
					<?php echo JHtml::fetch('vrehtml.admin.stateaction', $row['published'], $row['id'], 'payment.publish', $canEditState); ?>
				</td>
				...
				<?php if ($multi_lang && $canEdit): ?>
					<td style="text-align: center;">
						<a href="index.php?option=com_vikrestaurants&amp;view=langpayments&amp;id_payment=<?php echo (int) $row['id']; ?>">
							<?php
