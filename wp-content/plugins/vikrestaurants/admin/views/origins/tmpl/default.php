...
$canOrder     = $this->ordering == 'o.ordering';

if ($canOrder && $canEditState)
{
	$saveOrderingUrl = 'index.php?option=com_vikrestaurants&task=origin.saveOrderAjax&tmpl=component';
	JHtml::fetch('vrehtml.scripts.sortablelist', 'originsList', 'adminForm', $this->orderDir, $saveOrderingUrl);
}
 ...
 * called "onDisplayViewOriginsList". The event method receives the
...
<form action="index.php?option=com_vikrestaurants" method="post" name="adminForm" id="adminForm">
	...
	<div class="btn-toolbar" style="height: 32px;">
		...
		<!-- {"rule":"customizer","event":"onDisplayViewOriginsList","type":"search","key":"search"} -->
	...
	<div class="btn-toolbar hidden-phone" id="vr-search-tools" style="height: 32px;<?php echo ($is_searching ? '' : 'display: none;'); ?>">
		...
		<!-- Define role to detect the supported hook -->
		<!-- {"rule":"customizer","event":"onDisplayViewOriginsList","type":"search","key":"filters"} -->
...
else
{
	...
	<!-- Define role to detect the supported hook -->
	<!-- {"rule":"customizer","event":"onDisplayOriginsTableTH","type":"th"} -->

	<!-- Define role to detect the supported hook -->
	<!-- {"rule":"customizer","event":"onDisplayOriginsTableTD","type":"td"} -->
		...
		<?php echo $vik->openTableHead(); ?>
			<tr>
				...
				<th class="<?php echo $vik->getAdminThClass('left hidden-phone nowrap'); ?>" width="1%" style="text-align: left;">
					<?php echo JHtml::fetch('vrehtml.admin.sort', 'JGRID_HEADING_ID', 'o.id', $this->orderDir, $this->ordering); ?>
				</th>

				<!-- NAME -->
				
				<th class="<?php echo $vik->getAdminThClass('left'); ?>" width="15%" style="text-align: left;">
					<?php echo JHtml::fetch('vrehtml.admin.sort', 'VRMANAGELANG2', 'o.name', $this->orderDir, $this->ordering); ?>
				</th>

				<!-- ADDRESS -->

				<th class="<?php echo $vik->getAdminThClass('left hidden-phone'); ?>" width="30%" style="text-align: left;">
					<?php echo JHtml::fetch('vrehtml.admin.sort', 'VRCUSTFIELDRULE4', 'o.address', $this->orderDir, $this->ordering); ?>
				</th>
				...
				<th class="<?php echo $vik->getAdminThClass(); ?>" width="5%" style="text-align: center;">
					<?php echo JText::translate('VRMANAGEPAYMENT3'); ?>
				</th>
				
				<!-- ORDERING -->

				<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="1%" style="text-align: center;">
					<?php echo JHtml::fetch('vrehtml.admin.sort', '<i class="fas fa-sort"></i>', 'o.ordering', $this->orderDir, $this->ordering); ?>
				</th>
			...
			<tr class="row<?php echo ($i % 2); ?>">
				...
				<td>
					<div class="td-primary">
						...
						{
							?>
							<a href="index.php?option=com_vikrestaurants&amp;task=origin.edit&amp;cid[]=<?php echo (int) $row['id']; ?>">
								<?php echo $row['name']; ?>
							</a>
							<?php
						}
				...
				<td style="text-align: center;">
					<?php echo JHtml::fetch('vrehtml.admin.stateaction', $row['published'], $row['id'], 'origin.publish', $canEditState); ?>
				</td>
