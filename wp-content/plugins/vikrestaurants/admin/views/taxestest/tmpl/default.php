...
JHtml::fetch('vrehtml.scripts.selectflags', '#vre-lang-sel', array('width' => '90%', 'allowClear' => true));
		...
		<div class="row-fluid">
			...
			<div class="span6 full-width">
				<?php echo $vik->openEmptyFieldset(); ?>
					...
					<?php
					$taxes = JHtml::fetch('vrehtml.admin.taxes', $blank = '');

					echo $vik->openControl(JText::translate('VRETAXFIELDSET') . '*'); ?>
						<select name="id_tax" id="vre-taxes-sel" class="required">
					...
					<?php echo $vik->openControl(JText::translate('VRMANAGETKORDDISC5') . '*'); ?>
						<div class="input-prepend currency-field">
							<button type="button" class="btn"><?php echo VREFactory::getCurrency()->getSymbol(); ?></button>

							<input type="number" name="amount" value="<?php echo (float) $args['amount']; ?>" min="0" step="any" class="required" />
						</div>
					<?php echo $vik->closeControl(); ?>
					...
					echo $vik->openControl(JText::translate('VRMANAGELANG4')); ?>
						<select name="langtag" id="vre-lang-sel">
							<option></option>
					...
					<table class="rates-table table" id="tax-table" style="display: none;">
						<thead>
							<tr>
								...
								<th style="text-align: right;"><?php echo JText::translate('VRMANAGETKORDDISC5'); ?></th>
								<th style="text-align: right;"><?php echo JText::translate('VRMANAGEBILL2'); ?></th>
							</tr>
						</thead>
...
JText::script('VRINVTOTAL');
JText::script('VRINVTAXES');
JText::script('VRINVGRANDTOTAL');
?>
	...
	(function($) {
		...
		$(function() {
			...
			$('#test-tax-btn').on('click', function() {
				...
				UIAjax.do(
					'<?php echo $vik->ajaxUrl('index.php?option=com_vikrestaurants&task=tax.testajax'); ?>',
			...
			const buildTaxTable = (data) => {
				...
				tbody.append(createTableRow(Joomla.JText._('VRINVTOTAL'), null, data.net));
				...
				tfoot.append(createTableRow(Joomla.JText._('VRINVTAXES'), null, data.tax, {class: 'final-cost'}));
				// append total gross to footer
				tfoot.append(createTableRow(Joomla.JText._('VRINVGRANDTOTAL'), null, data.gross, {class: 'final-cost'}));
