...
JHtml::fetch('vrehtml.status.contextmenu', 'takeaway', '.status-hndl');
...
$listable_cf = VikRestaurants::getTakeAwayListableCustomFields();

// get all reservation codes
$allCodes = JHtml::fetch('vikrestaurants.rescodes', 2);
 ...
 * called "onDisplayViewTkreservationsList". The event method receives the
...
<form action="index.php?option=com_vikrestaurants" method="post" name="adminForm" id="adminForm">
	...
	<div class="btn-toolbar" style="height: 32px;">
		...
		<!-- {"rule":"customizer","event":"onDisplayViewTkreservationsList","type":"search","key":"search"} -->
		...
		<?php if ($config->getBool('tkenablestock')): ?>
			<div class="btn-group pull-right">
				<a href="index.php?option=com_vikrestaurants&view=tkstocks" class="btn">
					<i class="fas fa-search" style="margin-right: 4px;"></i>&nbsp;<?php echo JText::translate('VRTKSTOCKSOVERVIEWBTN'); ?>
				</a>

				<a href="index.php?option=com_vikrestaurants&view=tkstatstocks" class="btn">
					<i class="fas fa-chart-bar" style="margin-right: 4px;"></i>&nbsp;<?php echo JText::translate('VRTKSTATSTOCKSBTN'); ?>
				</a>
			</div>
		<?php endif; ?>
	...
	<div class="btn-toolbar hidden-phone" id="vr-search-tools" style="height: 32px;<?php echo ($is_searching ? '' : 'display: none;'); ?>">
		...
		<?php
		// get order statuses
		$options = JHtml::fetch('vrehtml.admin.statuscodes', 'takeaway', $blank = true);
		?>
		<div class="btn-group pull-left">
		...
		foreach ($this->supportedServices as $serviceId => $serviceName)
		{
			$options[] = JHtml::fetch('select.option', $serviceId, $serviceName);
		}
		?>
		<div class="btn-group pull-left">
		...
		<?php
		// get operators
		$options = JHtml::fetch('vikrestaurants.operators', $group = 2);
		...
		{
			// get working shifts
			$options = JHtml::fetch('vrehtml.admin.dayshifts', 2, $filters['date'], 'interval');
		...
		<!-- Define role to detect the supported hook -->
		<!-- {"rule":"customizer","event":"onDisplayViewTkreservationsList","type":"search","key":"filters"} -->
...
else
{
	...
	<!-- Define role to detect the supported hook -->
	<!-- {"rule":"customizer","event":"onDisplayTkreservationsTableTH","type":"th"} -->

	<!-- Define role to detect the supported hook -->
	<!-- {"rule":"customizer","event":"onDisplayTkreservationsTableTD","type":"td"} -->
			...
			<tr>
				...
				<?php if (in_array('sid', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('left hidden-phone'); ?>" width="10%" style="text-align: left;">
						...
						if (in_array('id', $listable_fields))
						{
							...
							echo JText::translate('VRMANAGETKRES2');	
						}
						else
						{
							// ID column hidden, add the possibility to sort by ID here
							echo JHtml::fetch('vrehtml.admin.sort', 'VRMANAGETKRES2', 'r.id', $this->orderDir, $this->ordering);
						}
						?>
					</th>
				<?php endif; ?>
				...
				<?php if (in_array('checkin_ts', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('left'); ?>" width="10%" style="text-align: left;">
						<?php echo JHtml::fetch('vrehtml.admin.sort', 'VRMANAGETKRES3', 'r.checkin_ts', $this->orderDir, $this->ordering); ?>
					</th>
				<?php endif; ?>

				<!-- SERVICE -->

				<?php if (in_array('delivery', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('left'); ?>" width="8%" style="text-align: left;">
						<?php echo JText::translate('VRMANAGETKRES13'); ?>
					</th>
				<?php endif; ?>
				...
				<?php if (in_array('customer', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('left'); ?>" width="10%" style="text-align: left;">
						<?php echo JHtml::fetch('vrehtml.admin.sort', 'VRMANAGETKRES24', 'r.purchaser_nominative', $this->orderDir, $this->ordering); ?>
					</th>
				<?php endif; ?>
				...
				<?php if (in_array('phone', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="8%" style="text-align: center;">
						<?php echo JText::translate('VRMANAGETKRES23'); ?>
					</th>
				<?php endif; ?>
				...
				<?php if (in_array('totpay', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('left hidden-phone'); ?>" width="8%" style="text-align: left;">
						<?php echo JHtml::fetch('vrehtml.admin.sort', 'VRMANAGETKRES8', 'r.total_to_pay', $this->orderDir, $this->ordering); ?>
					</th>
				<?php endif; ?>

				<!-- TAXES -->

				<?php if (in_array('taxes', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('left hidden-phone'); ?>" width="5%" style="text-align: left;">
				...
				<?php if (in_array('payment', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('left hidden-phone'); ?>" width="10%" style="text-align: left;">
						<?php echo JText::translate('VRMANAGETKRES27'); ?>
					</th>
				<?php endif; ?>
				...
				<?php if (in_array('coupon', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="5%" style="text-align: center;">
						<?php echo JText::translate('VRMANAGETKRES7'); ?>
					</th>
				<?php endif; ?>
				...
				<?php if (in_array('rescode', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="5%" style="text-align: center;">
						<?php echo JText::translate('VRMANAGETKRES26'); ?>
					</th>
				<?php endif; ?>
				...
				<?php if (in_array('status', $listable_fields)): ?>
					<th class="<?php echo $vik->getAdminThClass('left hidden-phone'); ?>" width="10%" style="text-align: left;">
						<?php echo JText::translate('VRMANAGETKRES9'); ?>
					</th>
				<?php endif; ?>
		...
		for ($i = 0; $i < count($rows); $i++)
		{
			...
			<tr class="row<?php echo ($i % 2); ?>">
				...
				<?php if (in_array('id', $listable_fields)): ?>
					<td class="hidden-phone text-nowrap">
						<div class="td-flex">
							...
							<a href="index.php?option=com_vikrestaurants&amp;view=rescodesorder&amp;id_order=<?php echo (int) $row['id']; ?>&amp;group=2" style="margin-left: 4px;">
				...
				<?php if (in_array('sid', $listable_fields)): ?>
					<td class="hidden-phone text-nowrap">
						<div>
							...
							{
								?>
								<a href="index.php?option=com_vikrestaurants&amp;task=tkreservation.edit&amp;cid[]=<?php echo (int) $row['id']; ?>">
									<?php echo $row['sid']; ?>
								</a>
								<?php
							}
				...
				if (in_array('checkin_ts', $listable_fields)) 
				{
					...
					if ($row['asap'])
					{
						$secondary .= '&nbsp;<i class="fas fa-shipping-fast hasTooltip" title="' . $this->escape(JText::translate('VRMANAGETKRESASAPSHORT')) . '"></i>';
					}
				...
				<?php if (in_array('delivery', $listable_fields)): ?>
					<td>
						<?php echo $this->supportedServices[$row['service']] ?? $row['service']; ?>
					</td>
				<?php endif; ?>
				...
				<?php if (in_array('customer', $listable_fields)): ?>
					<td>
						...
						<div class="mobile-only">
							<?php echo JHtml::fetch('vrehtml.status.display', $row['status'], 'badge'); ?>
						</div>
					</td>
				<?php endif; ?>
				...
				<?php if (in_array('totpay', $listable_fields)): ?>
					<td class="hidden-phone text-nowrap">
						...
						<?php if (!JHtml::fetch('vrehtml.status.ispaid', 'takeaway', $row['status']) && $row['total_to_pay'] > 0): ?>
							<div class="td-secondary">
								<?php
								// substract total paid from total gross
								echo JText::sprintf('VRORDERDUE', $currency->format(max(0, $row['total_to_pay'] - $row['tot_paid'])));
								?>
							</div>
						<?php endif; ?>
					</td>
				...
				<?php if (in_array('taxes', $listable_fields)): ?>
					<td class="hidden-phone">
						<?php echo $currency->format($row['total_tax']); ?>
					</td>
				<?php endif; ?>
				...
				<?php if (in_array('rescode', $listable_fields)): ?>
					<td style="text-align: center;" class="hidden-phone">
						...
						<?php echo JHtml::fetch('vrehtml.statuscodes.popup', 2, '#vrrescodelink' . $row['id']); ?>
					</td>
				<?php endif; ?>
...
<script>
	(function($, w) {
		...
		w.openCreditCardModal = (reservationId) => {
			let url = 'index.php?option=com_vikrestaurants&view=ccdetails&tmpl=component&tid=1&id=' + reservationId;
			openModal('ccdetails', url, true);
		}

		w.openCustomerModal = (customerId) => {
			let url = 'index.php?option=com_vikrestaurants&view=customerinfo&tmpl=component&locations=1&id=' + customerId;
			openModal('custinfo', url, true);
		}

		w.openOrderInfoModal = (reservationId) => {
			let url = 'index.php?option=com_vikrestaurants&view=tkorderinfo&tmpl=component&id=' + reservationId;
			openModal('respinfo', url, true);
		}

		w.openOrderHistoryModal = (reservationId) => {
			let url = 'index.php?option=com_vikrestaurants&view=orderhistory&tmpl=component&group=2&id=' + reservationId;
		...
		$(function() {
			...
			printDialog.addButton(Joomla.JText._('VROK'), (task, event) => {
				...
				$('#adminForm input[name="view"]').val('tkreservations');
