...
$canOrder     = $this->ordering == 'e.ordering';

if ($canOrder && $canEditState)
{
	$saveOrderingUrl = 'index.php?option=com_vikrestaurants&task=tkentry.saveOrderAjax&tmpl=component';
	JHtml::fetch('vrehtml.scripts.sortablelist', 'productsList', 'adminForm', $this->orderDir, $saveOrderingUrl, ['id_takeaway_menu' => $filters['id_takeaway_menu']]);
}
 ...
 * called "onDisplayViewTkproductsList". The event method receives the
...
<form action="index.php?option=com_vikrestaurants" method="post" name="adminForm" id="adminForm">
	...
	<div class="btn-toolbar vr-btn-toolbar" style="height:32px;">
		...
		<!-- {"rule":"customizer","event":"onDisplayViewTkproductsList","type":"search","key":"search"} -->
		...
		<div class="btn-group pull-right hidden-phone">
			<select name="id_takeaway_menu" id="vr-menu-sel" onchange="document.adminForm.submit();">
				<?php echo JHtml::fetch('select.options', JHtml::fetch('vikrestaurants.takeawaymenus'), 'value', 'text', $filters['id_takeaway_menu']); ?>
			</select>
		</div>
	...
	<div class="btn-toolbar hidden-phone" id="vr-search-tools" style="height: 32px;<?php echo ($is_searching ? '' : 'display: none;'); ?>">
		...
		<!-- Define role to detect the supported hook -->
		<!-- {"rule":"customizer","event":"onDisplayViewTkproductsList","type":"search","key":"filters"} -->
...
else
{
	...
	<!-- Define role to detect the supported hook -->
	<!-- {"rule":"customizer","event":"onDisplayTkproductsTableTH","type":"th"} -->

	<!-- Define role to detect the supported hook -->
	<!-- {"rule":"customizer","event":"onDisplayTkproductsTableTD","type":"td"} -->
		...
		<?php echo $vik->openTableHead(); ?>
			<tr>
				...
				<th class="<?php echo $vik->getAdminThClass('left hidden-phone'); ?>" width="1%" style="text-align: left;">
					<?php echo JHtml::fetch('vrehtml.admin.sort', 'JGRID_HEADING_ID', 'e.id', $this->orderDir, $this->ordering); ?>
				</th>

				<!-- NAME -->
				
				<th class="<?php echo $vik->getAdminThClass('left'); ?>" width="25%" style="text-align: left;">
					<?php echo JHtml::fetch('vrehtml.admin.sort', 'VRMANAGETKPRODUCT1', 'e.name', $this->orderDir, $this->ordering); ?>
				</th>

				<!-- PRICE -->
				
				<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="8%" style="text-align: center;">
					<?php echo JHtml::fetch('vrehtml.admin.sort', 'VRMANAGETKPRODUCT2', 'e.price', $this->orderDir, $this->ordering); ?>
				</th>
				...
				<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="8%" style="text-align: center;">
					<?php echo JText::translate('VRMANAGETKMENU9'); ?>
				</th>
				...
				<?php if ($multi_lang && $canEdit): ?>
					<th class="<?php echo $vik->getAdminThClass(); ?>" width="8%" style="text-align: center;">
						<?php echo JText::translate('VRMANAGETKPRODUCT4');?>
					</th>
				<?php endif; ?>
				...
				<th class="<?php echo $vik->getAdminThClass('hidden-phone'); ?>" width="1%" style="text-align: center;">
					<?php echo JHtml::fetch('vrehtml.admin.sort', '<i class="fas fa-sort"></i>', 'e.ordering', $this->orderDir, $this->ordering); ?>
				</th>
			...
			<tr class="row<?php echo ($i % 2); ?>">
				...
				<td>
					<div class="td-primary">
						...
						{
							?>
							<a href="index.php?option=com_vikrestaurants&amp;task=tkentry.edit&amp;cid[]=<?php echo (int) $row['id']; ?>">
								<?php echo $row['name']; ?>
							</a>
							<?php
						}
				...
				<td style="text-align: center;">
					<?php echo JHtml::fetch('vrehtml.admin.stateaction', $row['published'], $row['id'], 'tkentry.publish', $canEditState); ?>
				</td>

				<!-- PREPARATION -->

				<td style="text-align: center;" class="hidden-phone">
					<?php echo JHtml::fetch('vrehtml.admin.stateaction', $row['ready'], $row['id'], 'tkentry.ready', $canEditState); ?>
				</td>
				...
				<?php if ($multi_lang && $canEdit): ?>
					<td style="text-align: center;">
						<a href="index.php?option=com_vikrestaurants&amp;view=langtkproducts&amp;id_entry=<?php echo (int) $row['id']; ?>&amp;id_takeaway_menu=<?php echo (int) $filters['id_takeaway_menu']; ?>">
							<?php
				...
				<td style="text-align: center;" class="hidden-phone">
					<?php echo JHtml::fetch('vrehtml.admin.imagestatus', $row['img_path']); ?>
				</td>
...
?>
	...
	<input type="hidden" name="datasheet_type" value="takeawayproducts" />
	<input type="hidden" name="return" value="<?php echo base64_encode('index.php?option=com_vikrestaurants&view=tkproducts'); ?>" />
