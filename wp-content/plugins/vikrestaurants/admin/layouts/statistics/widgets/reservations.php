...
<script>
	...
	WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = function(widget, data, config) {
		...
		jQuery(widget).find('tr[data-resid]').each(function() {
			...
			if (id_res > LATEST_RESERVATION_FETCHED && firstDownload) {
				// update flag
			...
			if (!firstDownload  && id_res > LATEST_RESERVATION_FETCHED) {
				...
				jQuery(this).find('td')
					.first().find('.actions-group')
						.append(
							'<a href="javascript:void(0);" onclick="LATEST_RESERVATION_FETCHED = ' + id_res + ';jQuery(this).remove();">\n'+
