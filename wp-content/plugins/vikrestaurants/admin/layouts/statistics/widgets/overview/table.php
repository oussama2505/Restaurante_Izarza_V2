									...
									if (!$colspan || $table->multi_res)
									{
										...
										if ($info === null)
										{
											...
											else if (strtotime('+' . $filters['intervals'] . ' minutes', $ts) > $now)
											{
												...
												$link_href = 'index.php?option=com_vikrestaurants&task=reservation.add&date=' . $search->get('date') . '&hourmin=' . $hourmin->value . '&people=' . $table->min_capacity . '&id_table=' . $table->id . '&from=restaurant';
											}
											else
											{
												// date in the past
			...
			.click(function(event) {
				...
				if (OVERVIEW_CLOSURE_TARGET === null) {
					...
					new Promise((resolve, reject) => {
						...
						UIAjax.do(
							'index.php?option=com_vikrestaurants&task=closure.saveajax',
							{
