...
$params  = isset($displayData['params'])  ? $displayData['params']  : [];

$use_tags_highlighter = !empty($displayData['use_tags_highlighter']);

// get system editor (or the specified one)
$editor = VREApplication::getInstance()->getEditor($type);
...
// always reconstruct the ID for backward compatibility, since Joomla! 5.0 expects a valid ID
$id = preg_replace("/[^a-zA-Z0-9\-_]+/", '_', $name);

// display editor
echo $editor->display($name, $value, $width, $height, $rows, $cols, $buttons, $id, $asset = null, $author = null, $params);
