...
/**
 * VikRestaurants plugin Shortcodes model.
 * @wponly
 *
 * @since 	1.0
 * @see 	JModelForm
 */
class VikRestaurantsModelShortcodes extends JModelForm
{
	/**
	...
	public function all($columns = '*')
	{
		...
		if ($columns != '*')
		{
			...
			{
				$columns = implode(', ', array_map([$dbo, 'qn'], $columns));
			}
		}

		$q = "SELECT {$columns} FROM `#__vikrestaurants_wpshortcodes` ORDER BY `id` ASC";
	...
	public function best($views, $lang = null)
	{
		...
		do
		{
			...
			foreach ($shortcodes as $k => $v)
			{
				...
				if ($reqview === '*' || !strcasecmp($v->type, $reqview))
				{
