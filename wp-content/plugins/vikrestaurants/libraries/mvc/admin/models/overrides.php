...
/**
 * VikRestaurants plugin Overrides model.
 * @wponly
 *
 * @since 	1.2
 * @see 	JModel
 */
class VikRestaurantsModelOverrides extends JModel
{
	/**
	...
	protected function getViewsTree($client)
	{
		...
		{
			// admin path
			$path = VREADMIN . DIRECTORY_SEPARATOR . 'views';
		...
		{
			// site path
			$path = VREBASE . DIRECTORY_SEPARATOR . 'views';
		...
		$overridesPath = JPath::clean($upload['basedir'] . '/vikrestaurants/overrides/' . $clientFolder . '/');
		 ...
		 * @since 1.3
		 */
		if (JFolder::exists(VIKRESTAURANTS_LIBRARIES . '/mvc/' . $clientFolder . '/views'))
		{
			$views = array_merge($views, JFolder::folders(VIKRESTAURANTS_LIBRARIES . '/mvc/' . $clientFolder . '/views', '.', $recursive = false, $fullPath = true));
		}
	...
	protected function getLayoutsTree()
	{
		...
		$tree[] = [
			'name'   => 'admin',
			'path'   => VREADMIN . DIRECTORY_SEPARATOR . 'layouts',
			'folder' => true,
			'files'  => [],
		];
		...
		$tree[] = [
			'name'   => 'core',
			'path'   => VIKRESTAURANTS_LIBRARIES . DIRECTORY_SEPARATOR . 'html',
			'folder' => true,
			'files'  => [],
		];
		...
		// create base path of overrides
		$overridesPath = JPath::clean($upload['basedir'] . '/vikrestaurants/layouts/');
	...
	protected function getModulesTree()
	{
		...
		// create base path of overrides
		$overridesPath = JPath::clean($upload['basedir'] . '/vikrestaurants/overrides/modules/');

		// get all folders at the specified path
		$modules = JFolder::folders(VREMODULES, '.', $recursive = false, $fullPath = true);
		...
		foreach ($modules as $module)
		{
			...
			if (!$this->isExcluded($modName, $this->_excludedModules))
			{
				// create node to inject within the tree
				$node = [
					'name'   => preg_replace("/^mod_vikrestaurants_/i", '', $modName),
					'path'   => $module,
					'folder' => true,
					'files'  => [],
				];
	 ...
	 * In example, default_foo_bar.php is not part of VikRestaurants and therefore it
	 * will be discarded.
	 * 
	 * @return  array
	 * 
	...
	public function getAllOverrides()
	{
		...
		$lookup = [
			'admin'   => JFolder::files($upload['basedir'] . '/vikrestaurants/overrides/admin/', '\.php$', true, true),
			'site'    => JFolder::files($upload['basedir'] . '/vikrestaurants/overrides/site/', '\.php$', true, true),
			'layouts' => JFolder::files($upload['basedir'] . '/vikrestaurants/layouts/', '\.php$', true, true),
			'modules' => JFolder::files($upload['basedir'] . '/vikrestaurants/overrides/modules/', '\.php$', true, true),
		];
