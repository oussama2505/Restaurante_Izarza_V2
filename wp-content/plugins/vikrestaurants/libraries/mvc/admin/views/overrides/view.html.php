...
/**
 * VikRestaurants Overrides view.
 * @wponly
 *
 * @since 1.2
 */
class VikRestaurantsViewOverrides extends JView
{
	/**
	 * @override
	 * View display method.
		...
		if ($filters['path'])
		{
			...
			if ($supported)
			{
				...
				$filters['code'] = preg_replace_callback(
					"/\*\s*(\/administrator)?\/?components\/com_vikrestaurants(.*?)\R/i",
					function($match)
					{
						return '* /wp-content/plugins/vikrestaurants/'
							. ($match[1] ? 'admin' : 'site')
							. $match[2] . "\n";
					},
					$filters['code']
	...
	{
		JToolbarHelper::title(__('VikRestaurants - Page Overrides', 'vikrestaurants'));
