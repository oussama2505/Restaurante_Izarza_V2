...
<style>
	.vre-overrides-manager .overrides-body .override-file {
		display: flex;
		align-items: center;
		padding: 10px;
		border-bottom: 1px solid #ccc;
	}
	.vre-overrides-manager .overrides-body .override-file #override-options {
		font-size: 16px;
		margin-right: 10px;
	}
	.vre-overrides-manager .overrides-body .override-file input {
		flex: 1;
		margin-right: 4px;
	}

	.vre-overrides-manager .overrides-body .overrides-editor[data-layout="columns"] {
		display: flex;
		flex-wrap: wrap;
	}
	.vre-overrides-manager .overrides-body .overrides-editor[data-layout="columns"] > * {
	...
	.vre-overrides-manager .overrides-body .overrides-editor[data-layout="columns"] > *:only-of-type {
		width: 100%;
	}

	.vre-overrides-manager .overrides-body .overrides-editor .overrides-editor-default {
		visibility: hidden;
		position: fixed;
		transform: translateX(200%);
	}
	.vre-overrides-manager .overrides-body .overrides-editor[data-layout="columns"] .overrides-editor-default,
	.vre-overrides-manager .overrides-body .overrides-editor[data-layout="rows"] .overrides-editor-default {
		visibility: visible;
		position: initial;
		transform: none;
	}
	...
	.vre-overrides-manager .overrides-body .CodeMirror {
		height: 100%;
	}
</style>
...
<div class="overrides-editor">
	...
	<div class="overrides-editor-default">
		...
		echo $this->formFactory->createField()
			...
			->editor('codemirror')
			->name('__default')
			->value($this->filters['defaultcode'])
			->buttons(false)
			->hidden(true);
		?>
	</div>
		...
		$(function() {
			...
			Joomla.submitbutton = (task) => {
				if (task == 'override.save') {
					...
					// save code
					UIAjax.do(
						'admin-ajax.php?action=vikrestaurants&task=override.save',
						postData,
						(resp) => {
						...
						(error) => {
							...
							VREToast.dispatch(new VREToastMessageOS({
								body: error.responseText || Joomla.JText._('VRSYSTEMCONNECTIONERR'),
								status: VREToast.ERROR_STATUS,
							}));
			...
			$('#override-options').vikContextMenu({
				...
				buttons: [
					...
					{
						state: 1,
						text: '<?php echo addslashes(__('Show files tree', 'vikrestaurants')); ?>',
						separator: true,
					...
					// HIDE ORIGINAL CODE
					{
						text: '<?php echo addslashes(__('Hide original file', 'vikrestaurants')); ?>',
					...
					// SHOW IN COLUMNS
					{
						text: '<?php echo addslashes(__('Show both in columns', 'vikrestaurants')); ?>',
						action: saveOverrideLayoutMode,
						icon: isLayoutModeSelected,
						layoutMode: 'columns',
					},
					// SHOW IN ROWS
					{
						text: '<?php echo addslashes(__('Show one over the other', 'vikrestaurants')); ?>',
						action: saveOverrideLayoutMode,
						icon: isLayoutModeSelected,
						layoutMode: 'rows',
						separator: true,
					...
					// DELETE
					{
						...
						action: (root, event) => {
							...
							setTimeout(() => {
								...
								const r = confirm(Joomla.JText._('VRSYSTEMCONFIRMATIONMSG'));
