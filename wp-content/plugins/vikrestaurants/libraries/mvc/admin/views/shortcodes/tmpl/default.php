...
<form action="admin.php" method="post" name="adminForm" id="adminForm">
	...
	<div class="btn-toolbar" id="vr-search-tools" style="height: 32px;<?php echo ($is_searching ? '' : 'display: none;'); ?>">
		...
		?>
		<div class="btn-group pull-left">
			<select name="filter_type" id="vik-type-filter" class="<?php echo ($this->filters['type'] !== '' ? 'active' : ''); ?>" onchange="document.adminForm.submit();">
...
{
	echo $vik->alert(JText::translate('NO_ROWS_FOUND'));
}
else
{
	?>
	<table cellpadding="4" cellspacing="0" border="0" width="100%" class="<?php echo $vik->getAdminTableClass(); ?>" style="margin-top:10px;">
		...
		<?php foreach ($this->shortcodes as $i => $row): ?>
			<tr class="row">
				...
				<td style="text-align: center;">
					...
					{
						?>
						<a href="<?php echo get_permalink($row->post_id); ?>" target="_blank" class="hasTooltip" title="<?php echo $this->escape(JText::translate('VRE_SHORTCODE_VIEW_FRONT')); ?>">
					...
					else if ($row->tmp_post_id)
					{
						...
						?>
						<a href="edit.php?post_status=trash&post_type=<?php echo $post->post_type; ?>" target="_blank" class="hasTooltip" title="<?php echo $this->escape(JText::translate('VRE_SHORTCODE_VIEW_TRASHED')); ?>">
					...
					{
						?>
						<a href="javascript:void(0)" class="shortcode-add-post hasTooltip" title="<?php echo $this->escape(JText::translate('VRE_SHORTCODE_CREATE_PAGE')); ?>">
				...
				<td style="text-align: center;" class="hidden-phone">
					...
					else
					{
						...
						if (is_file(VREBASE . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'css' . DIRECTORY_SEPARATOR . 'flags' . DIRECTORY_SEPARATOR . $flag))
						{
							?>
							<img src="<?php echo VREASSETS_URI . 'css/flags/' . $flag; ?>" />
							<?php
						}
					}
					?>	
...
<?php
JText::script('VRE_SHORTCODE_CREATE_PAGE_CONFIRM');
?>
	...
	(function($, w) {
		...
		$(function() {
			$('.shortcode-add-post').on('click', function() {
				...
				if (!confirm(Joomla.JText._('VRE_SHORTCODE_CREATE_PAGE_CONFIRM'))) {
					// action denied
					return false;
				}
